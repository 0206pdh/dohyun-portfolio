...
const cloudLayers = [
  ...
  { title: "Security", detail: "IRSA로 Pod 단위 IAM 권한을 최소 범위로 부여하고, Secrets·ConfigMap으로 자격 증명과 설정 값을 애플리케이션 코드에서 분리했습니다." },
  ...
  return (
    <article className="troubleshooting-card">
      ...
      <div className="troubleshooting-copy">
        <p><b>문제</b>{item.problem}</p><p><b>해결</b>{item.action}</p>
      </div>
    </article>
  );
}
